...
void logger(string msg, LogType log_type = LogType::NONE)
{
...
private:
    ...
    uint64_t connection_id;
    chrono::system_clock::time_point last_connect_time;

    mt19937 rng;

    static constexpr chrono::duration<int> connect_timeout{TIMEOUT};
    static constexpr chrono::duration<int> connection_id_expire{CONNECTION_ID_EXPIRE};
...
public:
    TrackerManager(string url, uint16_t port) : url(url), port(port), sd(-1), connection_id(MAGIC_NUMBER), rng(chrono::system_clock::now().time_since_epoch().count()) {}
    ...
    optional<string> announce(vector<benc::byte_t>& src)
    {
...
int main(int argc, char** argv)
{
    ...
    //cout << boolalpha << tm.connect() << endl;
